...
//			ArduinoConnectModel.getInstance().removeEventListener(Event.CHANGE, onPlayerMove);
		...
		override public function onAdded(e:Event):void{
			mc_characterselect_copy.mc_timer.time = 1;
...
//			ArduinoConnectModel.getInstance().addEventListener(Event.CHANGE, onPlayerMove);
		}
		...
		protected function onPlayerMove(event:Event):void
		{
			...
			if (range1 > 0 && range1 < 120) {
				offset_y = mc_character_p1.height-80;
				range1 /= 120;
			...
			if (range2 > 0 && range2 < 120) {
				offset_y = mc_character_p2.height-80;
				range2 /= 120;
